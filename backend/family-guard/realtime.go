...
func (h *RealtimeHub) HandleWS(w http.ResponseWriter,r *http.Request){
    ...
    auth:=strings.TrimSpace(r.Header.Get("Authorization")); if !strings.HasPrefix(auth,"Bearer "){http.Error(w,"authorization required",401);return}
    if !validSession(deviceID,strings.TrimSpace(strings.TrimPrefix(auth,"Bearer "))){http.Error(w,"invalid session",401);return}
    c,err:=websocket.Accept(w,r,&websocket.AcceptOptions{CompressionMode:websocket.CompressionDisabled});if err!=nil{return}
    ...
    ctx:=r.Context();for{_,_,err:=c.Read(ctx);if err!=nil{return}}
}
func(h *RealtimeHub)Send(deviceID,event string,payload any){deviceID=strings.TrimSpace(deviceID);if deviceID==""||event==""{return};data,err:=json.Marshal(map[string]any{"event":event,"payload":payload,"sent_at":time.Now().UTC()});if err!=nil{return};h.mu.RLock();conns:=make([]*websocket.Conn,0,len(h.conns[deviceID]));for c:=range h.conns[deviceID]{conns=append(conns,c)};h.mu.RUnlock();for _,c:=range conns{ctx,cancel:=context.WithTimeout(context.Background(),5*time.Second);_=c.Write(ctx,websocket.MessageText,data);cancel()}}
